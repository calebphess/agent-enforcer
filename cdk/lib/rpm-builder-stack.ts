...
export class RpmBuilderStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const userDataScript = [
      ...
      '',
      '# Find and upload built RPM: stable latest key + versioned history',
      'RPM_FILE=$(find /root/rpmbuild/RPMS/ -name "*.rpm" | head -1)',
      'RPM_BASENAME=$(basename $RPM_FILE)',
      'RPM_VERSION=$(rpm -qp --queryformat "%{VERSION}" "$RPM_FILE")',
      `aws s3 cp "$RPM_FILE" "s3://${RPM_BUCKET}/installers/latest/agent-enforcer.rpm" --region "${this.region}"`,
      `aws s3 cp "$RPM_FILE" "s3://${RPM_BUCKET}/installers/$RPM_VERSION/$RPM_BASENAME" --region "${this.region}"`,
      '# Legacy flat key — older demo deployments look for *.rpm at the bucket root',
      `aws s3 cp "$RPM_FILE" "s3://${RPM_BUCKET}/$RPM_BASENAME" --region "${this.region}"`,
      `echo "SUCCESS: s3://${RPM_BUCKET}/installers/latest/agent-enforcer.rpm ($RPM_VERSION)"`,
      '',
      '# Upload build log',
